...
while, which is a Raptor Lake machine, a whole generation off. Read
`hardware/dell-precision-5690.nix` against this list before assuming anything in
it is still true of the board.
...
`mkOverride 990 true` is what settles it, which leaves only the bus IDs genuinely
ours to state.
...
- **i965 is dead weight on Xe-LPG, and the generic import is what installs it.**
  ...
  generation drops them, worth a couple of hundred MiB. Measure that with
  `nix store diff-closures` rather than `du` on the store paths, which misses
  what leaves alongside them. OpenCL is unaffected: Gen12+ is served by
...
- **`common-hidpi` was a no-op** and is no longer imported: since kernel 6.8 the
  console font is set by the kernel, so the module's only two settings sit behind
  a `versionOlder … "6.8"`.

- **The built-in camera works on this generation, and the blacklist is gone.**
  The board is `ov02e10` on a Meteor Lake IPU (`8086:7d19`), and libcamera turns
  it into a **`Built-in Front Camera`** PipeWire source that wireplumber makes
  the default video source. The 48 `Intel IPU6 ISYS Capture N` nodes are still
  there and are still junk, but wireplumber creates *no* source for them, so
  anything reaching the camera through PipeWire or the portal sees exactly three.
  Only a program enumerating `/dev/video*` itself sees the noise. Check
  `wpctl status` before believing a report that the camera is missing; the raw
  nodes appear under Devices and mean nothing.

## The fingerprint reader, and why only sudo gets it

Goodix `27c6:634c`, driven by libfprint's `goodixmoc`. It is a press pad, not a
swipe, so enrolling is 12 presses of `fprintd-enroll`.
...
- **greetd / login.** `pam_fprintd` is `sufficient` and sits at order 11400,
  ...
  out-of-nowhere password prompt that service exists to prevent.
- **swaylock.** Two things in swaylock's own `pam.c`: the auth worker only calls
  `pam_authenticate` *after* a submit, so the sensor is dead until you press
  Enter; and `PAM_TEXT_INFO`/`PAM_ERROR_MSG` both hit a bare `break`, so the
  "Place your finger…" prompt is discarded and nothing on screen says it is
  waiting. Revisit only with a short `timeout=`.
...
`timeout=30` and then returns `PAM_AUTHINFO_UNAVAIL` **once** rather than
looping, so it is 30s, not `max-tries` × 30s. `max-tries=3` burns down only on a
genuine no-match. Both are module arguments
(`security.pam.services.sudo.rules.auth.fprintd.args`) if 30s ever feels long.

sudo is the surface that suits it because **sudo-rs** prints the prompt to the
tty and leaves SIGINT live, so Ctrl+C drops straight to the password. zuko runs
sudo-rs, not sudo — but this needed no thought either way, since `security.sudo`
and `security.sudo-rs` declare the same two PAM service names.
...
know the GPU topology; Steam reaches the host command through `PATH`. A
single-GPU host has no `gpu-offload` at all, so a launch option copied there
fails loudly rather than silently rendering on the wrong card.
...
**Steam needs no `extraPackages` entry for this.** The FHS rootfs carries no
`/run` of its own, so bubblewrap auto-binds the host one, and the FHS
`/etc/profile` appends the host `$PATH` after `/usr/bin` — so anything in
`environment.systemPackages` is reachable from a launch option.
...
the vendor's offload variables. It was used here briefly. Three reasons it lost,
worth knowing before anyone reaches for it again:

- **It duplicates something already present.** zuko gets the NVIDIA script free
  from `enableOffloadCmd`; switcheroo adds a substantial closure plus a daemon
...
None of this replaces `hardware.nvidia.prime`, which is what makes offload
possible at all.
