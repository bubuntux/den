...
- **Imports**: Use `self.nixosModules` to reference other modules: `imports = with self.nixosModules; [ bundle-base gnome ];`
- **flake.nix**: Do not edit manually unless adding new top-level inputs; it is auto-generated by flake-file
- **Verification**: Run `nix flake check` after making changes to validate the configuration
